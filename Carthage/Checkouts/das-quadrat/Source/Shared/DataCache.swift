...
class DataCache {
        ...
        privateQueue.addOperation {
            ...
            if result == nil {
                ...
                if let result = result {
                    self.cache.setObject(result as AnyObject, forKey: key as NSString, cost: result.count)
                }
            }
        }
            ...
            if let values = values, let modificationDate = values.contentModificationDate {
                if (modificationDate as NSDate).laterDate(expirationDate) == modificationDate {
                    ...
                    if let fileSize = values.totalFileAllocatedSize {
                        cacheSize = cacheSize + UInt(fileSize)
                    }
                } else {
    ...
    fileprivate func sortFileURLByModificationDate(_ urls: [URL]) -> [URL] {
        return urls.sorted {
            ...
            do {
                let values1 = try url1.resourceValues(forKeys: Set(dateKey))
                let values2 = try url2.resourceValues(forKeys: Set(dateKey))
