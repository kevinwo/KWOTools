...
class DataTask: Task {
    override func constructURLSessionTask() {
        ...
        self.task = URLsession?.dataTask(with: request.URLRequest()) {
...
class UploadTask: Task {
    ...
    override func constructURLSessionTask() {
        ...
        var request = self.request.URLRequest()
        ...
        var pathExtension = self.fileURL!.pathExtension
        if pathExtension.isEmpty {
            pathExtension = "png"
        }

        appendStringBlock("\r\n--\(boundary)\r\n")
        appendStringBlock("Content-Disposition: form-data; name=\"photo\"; filename=\"photo.\(pathExtension)\"\r\n")
        appendStringBlock("Content-Type: image/\(pathExtension)\r\n\r\n")
        ...
        self.task = self.session?.URLSession.uploadTask(with: request, from: body) {
            (data, response, error) -> Void in
            self.session?.networkActivityController?.endNetworkActivity(self.networkActivityId)
