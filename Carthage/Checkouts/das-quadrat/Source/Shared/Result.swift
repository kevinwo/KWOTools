...
extension Result {
    ...
    class func createResult(_ HTTPResponse: HTTPURLResponse?, JSON: [String:AnyObject]?, error: Error? ) -> Result {
        let result = Result()
        if let error = error {
            result.error = error
            return result
    ...
    class func resultFromURLSessionResponse(_ response: URLResponse?, data: Data?, error: Error?) -> Result {
...
extension Result {
    ...
    public func isCancelled() -> Bool {
        if let error = self.error {
            return (error._domain == NSURLErrorDomain  && error._code == NSURLErrorCancelled)
        }
        return false
    }
}
