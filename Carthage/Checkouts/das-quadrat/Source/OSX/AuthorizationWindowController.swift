...
class AuthorizationWindowController: NSWindowController {
    ...
    func webView(_ sender: WebView!, didFailLoadWithError error: Error!, forFrame frame: WebFrame!) {
        self.status = .failed(error)
    }
    
    func webView(_ sender: WebView!,
        didFailProvisionalLoadWithError error: Error!, forFrame frame: WebFrame!) {
            self.status = .failed(error)
    }
